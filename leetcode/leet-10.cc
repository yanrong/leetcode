...
public:
    bool isMatch(string s, string p) {
        ...
        for(int j = 1; j <= l2; j++){
            //if the origin string is emtpy, only the * can match it
            //make sure the previous one is also matched
            dp[0][j] = j > 1 && p[j - 1] =='*' && dp[0][j - 2];
        }
        for(int i = 1; i <= l1; i++){
            for(int j = 1; j <= l2; j++){
                if(p[j - 1] != '*'){
                    //if the p[j - 1] is not '*', only the plain text is equal or p[j - 1] is '. and the previous data is match
                    dp[i][j] = (s[i - 1] == p[j - 1] || p[j - 1] == '.') && dp[i - 1][j - 1];
                }else{
                    //if the p[j - 1] is '*', * match the nothing or match one or more char
...
public:
    ...
    bool isMatch(string s, string p) {
        //empty patter only can match with empty origin.
