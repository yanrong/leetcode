...
public:
    ...
    bool isEqual(vector<int>& nums, int l, int r){
        for(int i = l; i < r; i++){
    ...
    void backTrace(vector<vector<int>>& ret, vector<int>& nums, int start, int depth){
        if(start == depth){
            ...
            for(int i = start; i <= depth; i++){
                 ...
                 **/
                if(isEqual(nums, start, i)) continue;
                swap(nums[start], nums[i]); //in recurse fixed each [start, depth] in index start
...
public:
    ...
    void backTrace(vector<vector<int>>& ret, vector<int>& nums, vector<bool>& mark, vector<int>& tmp, int depth){
        ...
        for(int i = 0; i < nums.size(); i++){
            //the data may have same data. mark[index] use key index to check if is available
            // !mark[i - 1] ??? 
